...
The research paper is written in LaTeX using the IEEE `IEEEtran.cls` journal class.

```bash
# Compile the paper (requires IEEEtran.cls in your TeX distribution)
pdflatex paper.tex
pdflatex paper.tex   # run twice for cross-references

# The paper references plot files in the plots/ directory
# Ensure plots/ is present relative to paper.tex
```

The paper includes the following figures from the `plots/` directory:

- `pvalue_heatmap_pharma.png` / `pvalue_heatmap_auto.png` — Cointegration p-value matrices
- `backtest_SUNDRMFAST__SUPRAJIT.png` — Best pair backtest
- `rolling_hl_SUNDRMFAST__SUPRAJIT.png` — Rolling half-life with event markers
- `regime_comparison.png` — Pre/post event half-life comparison
- `spread_TORNTPHARM_ALKEM.png` — Spread visualization
- `basket_backtest_AUROPHARMA__TORNTPHARM__ABBOTINDIA.png` — Best basket backtest
- `basket_regime_comparison.png` — Basket regime analysis
...
```bibtex
@article{kothari2024meanreversion,
  author  = {Yash Kothari},
  title   = {Mean Reversion Compression in {Indian} Equities: A Cointegration Analysis of Pairs and Baskets},
  journal = {IEEE Transactions on Quantitative Finance},
  year    = {2024}
}
```
...
- National Stock Exchange of India for providing the data infrastructure
- Yahoo Finance API via `yfinance` for historical price data
- The `statsmodels` library for cointegration testing implementations
